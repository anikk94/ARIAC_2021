...
class WorldModel:
    ...
    def __init__(self):
        ...
        # agv position
        self.agv1_station = None
        self.agv2_station = None
        self.agv3_station = None
        self.agv4_station = None

        self.robot = Robot(world_model = self)
        ...
        try:
            ...
            thread_create_csnapshot_parts = Thread(target=self._create_csnapshot_parts)
            thread_create_csnapshot_parts.daemon = True
            thread_create_csnapshot_parts.start()
    ...
    def add_conveyor_part(self, part):
        """Adds the part to the world model
        Args:
           part: Part to be added
        """
    ...
    # def add_assembly_part(self,part):
    #     #print('adding assembly world model')
    ...
    def _create_csnapshot_parts(self):
        """The thread to create the snapshot of all the conveyor parts we see by the conveyor logical cameras
        """
        while not rospy.is_shutdown():
            # print_partition()
            # rospy.loginfo("Faulty Parts:")
            self.conveyor_parts = []
            for model_data in self.csnapshot.values():
                for part in model_data:
                    self.add_conveyor_part(part)
                    # rospy.loginfo(part.location_type)
    ...
    def find_assembly_parts(self, part):
        part_flag_found = False
        part_found = None
        assembly_agv=None
        for agv in self._agv_robots.values():
            part_flag_found, part_found = agv.find_assembly_parts(part)
            if part_flag_found:
                assembly_agv=agv.agv_id
                break
        
        return (part_flag_found, part_found, assembly_agv)
    ...
    def search_part_in_conveyor(self, part_type):
        flag = False
        part_found = None
        rospy.loginfo("search_part_in_conveyor")
        rospy.loginfo(self.conveyor_parts)
